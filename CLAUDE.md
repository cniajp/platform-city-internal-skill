...
6. **クロスプラットフォームで書く**。コマンド例は macOS / Linux 両対応にする。**Windows は WSL2（Linux 環境）前提**とし、PowerShell ネイティブ手順は書かない（内容は Linux と同等）。OS 依存はインストール手順くらいに留め、本筋の手順は共通のシェルコマンドで書く。
